...
const authApi = (action, body = {}) => {
  ...
  return fetch('auth.php', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,...body})}).then(async r=>{const d=await r.json();if(!r.ok||d.error)throw new Error(d.error||'Request failed');return d});
};
const profileApi = (action, body = {}) => {
  if (window.GIZMO?.profileApi) return window.GIZMO.profileApi(action, body);
  return fetch('profiles.php', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,...body})}).then(async r=>{const d=await r.json();if(!r.ok||d.error)throw new Error(d.error||'Request failed');return d});
};
