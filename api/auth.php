<?php
/**
 * Auth API — Works for Vercel (/api/auth.php) and XAMPP (root/auth.php)
 */
...
if ($action === 'updateStats') {
    ...
    if (!$id) json_reply(['error' => 'User id is required.'], 400);

    update_streak($db, $id);

    $db->prepare(
        'UPDATE users SET total_score = total_score + ?, total_games = total_games + ?,
...
if ($action === 'updatePhoto') {
    ...
    if (!$id || !$photo) json_reply(['error' => 'User id and photo are required.'], 400);
...
if ($action === 'addKnown') {
    ...
    if ($knownUserId) {
        $db->prepare('INSERT IGNORE INTO user_known_players (user_id, known_user_id) VALUES (?, ?)')
            ->execute([$userId, $knownUserId]);
    } else {
        $db->prepare('INSERT IGNORE INTO user_known_players (user_id, known_name) VALUES (?, ?)')
...
if ($action === 'social') {
    ...
    $following = $db->prepare(
        'SELECT u.id, u.name, u.photo, u.updated_at FROM user_follows f
         JOIN users u ON u.id = f.following_id WHERE f.follower_id = ? ORDER BY u.name'
    );
    $following->execute([$id]);

    $followers = $db->prepare(
        'SELECT u.id, u.name, u.photo, u.updated_at FROM user_follows f
         JOIN users u ON u.id = f.follower_id WHERE f.following_id = ? ORDER BY u.name'
    );
    ...
    $known = $db->prepare(
        'SELECT COALESCE(u.name, k.known_name) AS name, k.known_user_id AS userId,
         ...
         WHERE k.user_id = ? ORDER BY name LIMIT 20'
    );
    $known->execute([$id]);

    $mapPeople = fn($rows) => array_map(fn($row) => format_person($row), $rows);
